...
function isBodyInViewport(body, x, y, w, h) {
  return (
    body.bounds.max.y <= h + y &&
    body.bounds.max.y >= y
  )
};
...
function preprocessPage(bodyElement) {
  const textNodes = getTextNodes(bodyElement);
  let benderTextNodes = []
  textNodes.forEach(node => {
    if (node.nodeValue.trim().length != 0
      && node.parentElement
      && shouldWrapElement(node.parentElement)) {
      if (!benderTextNodes.includes(node.parentElement)) {
        benderTextNodes.push(node.parentElement)
      }
    }
  })
  const toWrap = []
  benderTextNodes.forEach(e => {
    let elementNotIncluded = true
    benderTextNodes.forEach(e2 => {
      if (e2 !== e && e2.contains(e)) {
        elementNotIncluded = false
      }
    })
    if (elementNotIncluded) {
      toWrap.push(e)
    }
  })
  toWrap.forEach(e => {
    wrapWordsInElement(e)
  })
  const words = Array.from(document.body.querySelectorAll('.bender-word'))

  const images = getImagesInElement(document.body).filter(i => shouldWrapElement(i))
  return {
    words,
    images
  }
...
function wrapWordsInElement(e) {
  if (!e.classList.contains('bender-word')) {
    e.innerHTML = e.innerHTML.replace(/(?<!(<\/?[^>]*|&[^;]*))([^\s<]+)/g, '$1<span style = "display: inline; margin: 0; padding:0;" class="bender-word bender-selectable">$2</span>')
  }
  const wrappedWords = Array.from(e.querySelectorAll('.bender-word'))
  return wrappedWords
}

function getImagesInElement(element) {
  const imgTags = element.querySelectorAll('img')
  const images = Array.from(imgTags)
  return images
}

function copyWordElementForPhysics(element) {
  const newElement = createNewElementFrom(element)
  newElement.innerText = element.innerText
  // newElement.style.width = 'unset'
  //newElement.style.height = 'unset'
  return {
...
function copyImgTagElementForPhysics(element) {
  const newElement = createNewElementFrom(element)
...
function createNewElementFrom(element) {
  ...
  newElement.style.transform = `translate3d(${bounds.x + window.scrollX}px, ${bounds.y + window.scrollY}px, 0)`
  newElement.style.transformOrigin = 'center'
  newElement.style.zIndex = 9999
  newElement.style.margin = "0 0 0 0"
  newElement.style.padding = "0 0 0 0"
  newElement.style.border = "0 0 0 0"
  newElement.style.transition = "all 0s"
  newElement.style.WebkitTransition = "all 0s"
  newElement.style.minWidth = 'unset'
  newElement.style.minHeight = 'unset'
  newElement.style.maxWidth = 'unset'
  newElement.style.maxHeight = 'unset'
  newElement.style.willChange = 'transform'
  newElement.style.fontStyle = computedStyle.fontStyle
  newElement.style.fontVariant = computedStyle.fontVariant
  ...
  newElement.style.lineHeight = computedStyle.fontSize
  newElement.style.textTransform = computedStyle.textTransform
  newElement.style.color = computedStyle.color
  return newElement
}

function copyBackgroundImageElementForPhysics(element) {
  newElement = createNewElementFrom(element)
  return {
    physicsElement: newElement,
    originalElement: element,
...
function copyElementForPhysics(element) {
  console.log("1")
  const existingElement = window.benderPhysics.physicsElements.find(e => e.originalElement === element)
  if (existingElement) {
    return {
      existingElement
    }
  ...
  if (element.classList.contains('bender-word')) {
    console.log("3")

    return copyWordElementForPhysics(element)
  }
  if (element.tagName === 'IMG') {
    return copyImgTagElementForPhysics(element)
  }
  const style = window.getComputedStyle(element)
  if (style.backgroundImage !== 'none') {
    return copyBackgroundImageElementForPhysics(element)
  }

}

function createBoundaryWalls(wallThickness) {
  const top = Matter.Bodies.rectangle(window.scrollX + window.innerWidth / 2,
    window.scrollY - wallThickness / 2,
    window.innerWidth * 2,
    wallThickness,
    { isStatic: true, label: 'top' })
  const bottom = Matter.Bodies.rectangle(window.scrollX + window.innerWidth / 2,
    window.scrollY + window.innerHeight + wallThickness / 2 - 10,
    window.innerWidth * 2,
    wallThickness,
    { isStatic: true, label: 'bottom' })
  const left = Matter.Bodies.rectangle(window.scrollX - wallThickness / 2,
    window.scrollY + window.innerHeight / 2,
    wallThickness,
    window.innerHeight * 2,
    { isStatic: true, label: 'left' })
  const right = Matter.Bodies.rectangle(window.scrollX + window.innerWidth + wallThickness / 2 - 20,
    window.scrollY + window.innerHeight / 2,
    wallThickness,
    window.innerHeight * 2,
  ...
  {
    body: bottom,
    targetPosition: bottom.position,
    name: 'bottom'
  },
  {
    body: left,
...
function customizeMatterEngine() {
  // Add optional 'ignoreGravity' property to bodies
...
function setupMatterEngine(gravity = { x: 0, y: 0 }) {
  customizeMatterEngine()
...
function createBenderPhysics(gravity = { x: 0, y: 0 }) {
  const wallThickness = 600
  const pageData = preprocessPage(document.body)
  const engine = setupMatterEngine(gravity)
  const boundaryWalls = createBoundaryWalls(wallThickness)
  Matter.Composite.add(engine.world, boundaryWalls.map(w => w.body))
      ...
      switch (w.name) {
        case 'top':
          w.targetPosition = {
            ...
            y: window.scrollY - wallThickness / 2
          }
          break
        case 'bottom':
          w.targetPosition = {
            x: window.scrollX + window.innerWidth / 2,
            y: window.scrollY + window.innerHeight + wallThickness / 2 - 10
          }
          break
        case 'left':
          w.targetPosition = {
            x: window.scrollX - wallThickness / 2,
            y: window.scrollY + window.innerHeight / 2
          }
          break
        case 'right':
          w.targetPosition = {
            x: window.scrollX + window.innerWidth + wallThickness / 2 - 20,
            y: window.scrollY + window.innerHeight / 2
          }
          break
      }
  ...
  function tick(deltaTime = 16.6667) {

    Matter.Engine.update(engine, deltaTime)
    boundaryWalls.forEach(wall => {
      const { body, targetPosition } = wall
      const deltaX = targetPosition.x - body.position.x
      const deltaY = targetPosition.y - body.position.y
      Matter.Body.setStatic(body, false)
      Matter.Body.translate(body, { x: deltaX / 10, y: deltaY / 10 })
  ...
  function start() {
    if (!window.benderPhysics.tickInterval) {
      window.benderPhysics.interval = setInterval(tick, 16.66)
    }
  }
  function stop() {
    if (window.benderPhysics.tickInterval) {
      clearInterval(window.benderPhysics.interval)
    }
  }
  function reset() {
  ...
  function addElements(elements, isStatic) {
    let physicsDiv = document.querySelector('#bender-physics')
    if (!physicsDiv) {
      physicsDiv = document.createElement('div')
      physicsDiv.id = 'bender-physics'
      physicsDiv.style.position = 'absolute'
      physicsDiv.style.top = '0'
      physicsDiv.style.left = '0'
      document.body.appendChild(physicsDiv)
    }
    let newElements = elements.map(element => copyElementForPhysics(element))
    console.log("newElements", newElements)
    const existingElements = newElements.filter(e => e.existingElement)
    console.log("existingElements", existingElements)
    newElements = newElements.filter(e => !e.existingElement)
    if (newElements.length === 1) {
      const newElement = newElements[0]
      physicsDiv.append(newElement.physicsElement)
    } else {
      const fragment = document.createDocumentFragment();
      newElements.forEach(e => {
        fragment.appendChild(e.physicsElement)
      })
      physicsDiv.appendChild(fragment)
    }

    newElements.forEach(e => {
      const newElement = e.physicsElement
      const bounds = newElement.getBoundingClientRect()
      const bodyBounds = document.body.getBoundingClientRect()
      const x = window.scrollX + bounds.x + bounds.width / 2
      const y = window.scrollY + bounds.y + bounds.height / 2
      const originalElement = e.originalElement
      ...
      if (e.type === 'img' || e.type === 'word') {
        originalElement.style.visibility = 'hidden'
      } else if (e.type === 'background-image') {
        originalElement.style.backgroundImage = 'none'
      }
      benderPhysics.physicsElements.push({
        physicsBody: box,
    ...
    existingElements.forEach(e => {
      Matter.Body.setStatic(e.existingElement.physicsBody, isStatic)
    })
  }
...
function setTransform(e) {
  let currentTransform = e.domElement.style.transform
  let newTransform = `translate3d(${parseFloat((e.physicsBody.position.x - e.width / 2).toFixed(2))}px, ${parseFloat((e.physicsBody.position.y - e.height / 2).toFixed(2))}px, 0px) rotateZ(${e.physicsBody.angle}rad)`
  if (currentTransform !== newTransform) {
    e.domElement.style.transform = newTransform
  }
}

function render() {
  const n = window.benderPhysics.physicsElements.length
  const elements = window.benderPhysics.physicsElements
  ...
  for (let i = 0; i < n; i++) {
    const e = elements[i]
    if (isBodyInViewport(e.physicsBody, x, y, w, h)) {
      setTransform(e)
    }
  }
}

function findOrCreateBenderPhysics() {
  if (window.benderPhysics) {
    return window.benderPhysics
  } else {
    window.benderPhysics = createBenderPhysics({ x: 0, y: 1 })
    return window.benderPhysics
  }
}

  window.benderPhysics = findOrCreateBenderPhysics()
  const words = benderPhysics.pageData['words'].filter(e => isInViewport(e))
  benderPhysics.addElements(words, true)
  benderPhysics.physicsElements.forEach(e => {
    e.physicsBody.collisionFilter.group = -1
    e.domElement.style.backgroundColor = backgroundColor='rgba(50,50,50,.3)'
  ...
  */
  ...
  function addCharacter() {
    const engine = benderPhysics.engine
    ...
    //  circle.style.height = `${playerRadius*2}px`
    // circle.style.width = `${playerRadius*2}px`
    const circle = document.createElement('span')
    circle.id = 'player-original'
    circle.classList.add('bender-word')
    circle.style.position = 'absolute'
    circle.style.top = '100px'
    circle.style.fontSize = `${playerRadius * 2}px`
    circle.style.left = '100px'
    circle.style.lineHeight = `${playerRadius * 2}px`
    circle.innerText = '😺'
    document.body.appendChild(circle)
    benderPhysics.addElements([circle])
    benderPhysics.physicsElements[benderPhysics.physicsElements.length - 1].domElement.style.backgroundColor = 'transparent'
    let player = benderPhysics.physicsElements[benderPhysics.physicsElements.length - 1].physicsBody
    Matter.Body.scale(player, .5, .8)
    ...
    benderPhysics.engine.gravity.y = .45
    const runSpeed = .00075
    const jumpForce = .0022
    let canJump = true
    Events.on(engine, "beforeUpdate", function (event) {
      ...
      //jump
      if (keysDown[38] && player.ground && canJump) {
      ...
      //spin left and right
      const limit = 2.5;
      if (keysDown[39] && player.velocity.x < limit) {
        Matter.Body.applyForce(player, player.position, { x: runSpeed, y: 0 })
      } else {
        if (keysDown[37] && player.velocity.x > -limit) {
          Matter.Body.applyForce(player, player.position, { x: -runSpeed, y: 0 })
        }
      };
    });

    const senssorSize = Math.abs(player.bounds.max.x - player.bounds.min.x) * .99

    var playerSensor = Bodies.rectangle(0, 0, senssorSize, 5, {
      isSensor: true
      //isStatic: true,
    })
    playerSensor.collisionFilter.group = -2
    player.collisionFilter.group = -2
    Matter.Composite.add(engine.world, [player, playerSensor])
    ...
    function playerGroundCheck(event, ground) { //runs on collisions events
      ...
      for (var i = 0, j = pairs.length; i != j; ++i) {
        var pair = pairs[i];
        if (pair.bodyA === playerSensor && pair.bodyB !== player) {
          if (ground){
            player.ground = pair.bodyB;
          } else {
            player.ground = false
          }
        } else if (pair.bodyB === playerSensor && pair.bodyA !== player) {
          if (ground){
            player.ground = pair.bodyA;
          } else {
            player.ground = false
          }
        }
    ...
    function checkForBonk(event) {
      var pairs = event.pairs
      for (var i = 0, j = pairs.length; i != j; ++i) {
        const pair = pairs[i]
        if (pair.bodyA === playerSensor || pair.bodyB === playerSensor) {
        ...
        if (ground && ground.collisionFilter.group === -1 && ground !== player.ground) {
          Matter.Body.setStatic(ground, false)
          ground.collisionFilter.group = 0
          benderPhysics.physicsElements.find(e => e.physicsBody === ground)
            .domElement.style.backgroundColor = backgroundColor='rgba(250,50,50,.3)'
            player.velocity.y = 0
    ...
    Events.on(engine, "afterUpdate", function (event) {
      ...
      Matter.Body.setPosition(playerSensor, {
        x: player.position.x,
        y: player.position.y + playerRadius
      });
      console.log("player.ground", player.ground)
      if (!player.ground){
